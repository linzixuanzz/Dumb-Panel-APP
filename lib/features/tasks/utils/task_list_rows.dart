...
// 都建立在「全部任务都在手上」这个前提上，改成增量分页会把它们**算错**。
// 所以这一轮只修渲染，不动取数。
// （当时拖拽排序也在这个前提上：逐条写 sort_order，分页会把没加载到的任务写坏。
// v1.3.8 起改走 `PUT /tasks/sort`，兄弟序列由面板按整桶去取，这一条已不成立。）
...
/// 按任务标签里的「分组:」前缀分桶。
///
/// 组内任务保持服务端给的原始顺序（置顶、状态分区、拖拽顺序 list_order 都已在后端排好），
