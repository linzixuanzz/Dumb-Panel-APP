...
v1.3.8 跟随面板 v3.3.3（#147 #149 #150）加了批量设置通知、日志打开位置偏好、镜像源对齐，并修了拖拽排序写错字段，
下面是那一轮刻意留下的：

- **没有真机 / 模拟器端到端实测**。批量通知、`PUT /tasks/sort`、日志打开位置偏好都只有 FakeHttpAdapter 级别的测试，
  「连 v3.3.3 / v3.3.2 / v3.3.0 三种面板」的手测没做。
- **日志打开位置开关在 PUT 飞行中可以再点一次**，会按界面上的新状态再发一个 PUT。面板按键串行合并，回显总等于本次写入值，
  不会误报「不支持」，最坏只是两次请求都回来前勾选状态短暂与服务端不一致。
- **「收到 done 后不重排」只靠注释与代码审查守着**：`log_stream_page.dart` 的 `_follow` 只在首次加载定一次，
  测试里起不了 SSE，这条约束没有自动化覆盖。
- **Node 原生扩展换镜像（Alpine ↔ Debian）后的 libc 缺口是面板侧的事**，面板 v3.3.3 只修了 Python，Node 另开任务。
...
v1.3.6 修了 issue #10（实时日志只在底部跟随、大日志不卡），下面是那一轮刻意留下的：

- **还有三处日志视图没换成共用的 LogView**：依赖安装日志、订阅拉取日志、脚本调试输出。
  它们目前日志量不大、也没有「运行中持续追加」的场景，暂未迁移；以后迁移时直接复用
  `lib/shared/widgets/log_view.dart`，不要再写第四份滚动逻辑。
- **没有在真机上验证**：懒渲染之后跨行长按选择 / 复制的体验、「回到底部」按钮在自定义日志背景上的观感，
  以及大任务真实 SSE 流下的流畅度，都只有 widget 测试佐证。
